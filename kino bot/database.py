...
# --- Movies ---
async def add_movie(code: str, title: str, movie_type: str, file_id: str):
    try:
        await db.execute("INSERT INTO movies (code, title, movie_type, file_id) VALUES (?, ?, ?, ?)", 
                         (code, title, movie_type, file_id))
        await db.commit()
        return True
...
async def add_channel(channel_id: int, url: str, name: str):
    try:
        await db.execute("INSERT INTO channels (channel_id, url, name) VALUES (?, ?, ?)", (channel_id, url, name))
        await db.commit()
        return True
    except:
        return False
